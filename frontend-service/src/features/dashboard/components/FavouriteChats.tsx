...
export function FavouriteChats() {
  ...
  if (!loaded || peers.length === 0) return null;
    ...
    >
      <div className="flex items-center justify-between px-5 py-4 border-b" style={{ borderColor: "var(--border)" }}>
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-bold text-sm" style={{ color: "var(--text-primary)" }}>Favourites</h3>
        </div>
        <span className="text-[11px] font-medium px-2 py-0.5 rounded-full" style={{ background: "var(--row-hover-bg)", color: "var(--text-muted)" }}>
          {peers.length}
        </span>
      </div>

      <div className="p-3 flex gap-4 overflow-x-auto custom-scrollbar">
        {peers.map((peer, i) => (
